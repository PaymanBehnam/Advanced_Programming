...
ECE_Ghost::ECE_Ghost(color ghostCol) {
	...
	dirX = 0;
	dirY = 0;
	// zCoord doesn't change! we set it here. But xCoord and yCoord are related with xLoc and yLoc. So they are calculated from xLoc and yLoc and initialized in setLoc() function
	zCoord = 0;
	speed = 0.08;
	started = false;
	chase = true;
...
void ECE_Ghost::move()
{
	...
	ghostMap[getRow()][getCol()] = 0;

	// update position of ghost
	updatePosition(&xLoc, &yLoc, dirX, dirY, speed, true);
...
////texture for ghost has been disabled in this lab!
/*
