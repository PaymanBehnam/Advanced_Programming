...
Description: .cpp for game_control. 
*/
#include <iostream>
#include <string>
#include <GL/glut.h>
...
void GameControl::start() {
	started = true;
	std::cout << "Lives: " << pacmanLives << "  Scores: " << gainedCoins << std::endl; //This works in mac version
}
...
void GameControl::update(int delta) {
	...
	for (int i = 0; i < 4; i++) {
		...
		if (maze[pacman->getRow()][pacman->getCol()] == 1) {
			...
			remainedCoins--;
			gainedCoins++;
			std::cout << "Scores: " << gainedCoins << std::endl;
		}
...
//Cout at the end of the game when run in terminal: works properly in mac
...
void GameControl::drawText() {
	...
	for (long unsigned int i = 0; i < liveString.length(); i++) {
